...
int move_to_closest_target(TargetInfo *target_info) {
    ...
    if (abs(dx) > TARGET_DEADZONE || abs(dy) > TARGET_DEADZONE) {
        ...
        if (abs(dx) > SCREEN_CENTER_X/2) {
            horizontal_adjustment = 3; // Far from center, move 3 degrees
        } else {
            horizontal_adjustment = 2; // Closer to center, move 2 degrees
        }

        // Vertical adjustment
        if (abs(dy) > SCREEN_CENTER_Y/2) {
            vertical_adjustment = 3; // Far from center, move 3 degrees
        } else {
            vertical_adjustment = 2; // Closer to center, move 2 degrees
        }
